...
logWith сonf logLevel text = do
  ...
  if logLevel >= minLevel
    then do
      today <- currentDay
      time <- currentTime
      let message = show today ++ " " ++ time ++ " [" ++ show (confMinLevel сonf) ++ "] " ++ "[" ++ show logLevel ++ "] " ++ T.unpack text -- BC.unpack (Data.Text.Encoding.encodeUtf8 text)
...
currentTime = do
  ...
  let (TM.TimeOfDay hour minute sec) = TM.localTimeOfDay $ TM.utcToLocalTime timezone now
